...
{
    if(data.last_read_index_face == grid.visible_fields.Len() &&
        data.last_read_index_face_flags == grid.flags.Len()) return true;
    ...
    data.last_read_index_face_flags = grid.flags.Len();
    data.face = face_new;
